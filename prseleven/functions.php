...
<?php
			...
			if($numrows !=0)
			{
				...
				if($username == $dbusername && passwordCheck($password, $dbpassword))
				{
                    ...
                    if($type=="admin"){
                        ...
                        header("Location:verifyclient.php");
                    }
                    return true;
				}
			}
    ...
    function signup($fullname,$name,$email,$password,$phone,$repassword,$gender,$type,$conn){
        $hashedPassword = encryption($password);

        $query = mysqli_query($conn,"INSERT INTO users(fullname, name, email, phone, password, gender, type) VALUES('$fullname','$name','$email','$phone','$hashedPassword','$gender','$type')");
        ...
        if($query){
            ...
            header("Location:gfactor.php");
            exit;
        }
        else{
            echo "Query Error : " . "INSERT INTO users(fullname, name, email, phone, password, gender, type) VALUES('$fullname','$name','$email','$phone','$hashedPassword','$gender','$type')" . "<br>" . mysqli_error($conn);
            echo "<br>";
